...
RUN apt-get update && \
    ...
    rm -rf /var/lib/apt/lists/*
RUN mkdir -p /usr/local/rust-1.40.0/bin && \
    ln -sf $(which rustc) /usr/local/rust-1.40.0/bin/rustc
...
RUN apt-get update && \
    ...
    rm -rf /var/lib/apt/lists/*
RUN mkdir -p /usr/local/go-1.13.5/bin && \
    ln -sf $(which go) /usr/local/go-1.13.5/bin/go
...
RUN mkdir -p /usr/local/php-7.4.1/bin && \
    ln -sf $(which php) /usr/local/php-7.4.1/bin/php
...
RUN mkdir -p /usr/local/kotlin-1.3.70/bin && \
    ln -sf $(which kotlinc) /usr/local/kotlin-1.3.70/bin/kotlinc && \
    ln -sf $(which kotlin) /usr/local/kotlin-1.3.70/bin/kotlin
